/*--------------------------------------------------------------------
  版权 (C) 2015-2019, NXP B.V.
  保留所有权利。
  
  在符合以下条件的情况下，允许以源代码和二进制形式进行重新分发和使用，
  无论是修改过还是未修改过的：
  
  1. 源代码的重新分发必须保留上述版权声明、此条件列表和以下免责声明。
  
  2. 二进制形式的重新分发必须在随分发提供的文档和其他材料中复制上述版权声明、
     此条件列表和以下免责声明。
  
  3. 未经事先书面许可，不得使用版权持有人或其贡献者的名字来认可或推广从
     此HDL软件衍生的产品。
  
  本(HDL)软件由版权持有人和贡献者"按原样"提供，任何明示或暗示的保证，
  包括但不限于适销性和特定用途适用性的暗示保证，均被拒绝。在任何情况下，
  版权持有人或贡献者对任何直接、间接、偶然、特殊、典型或后果性损害
  （包括但不限于替代商品或服务的采购；使用、数据或利润的损失；或业务中断）
  均不负责任，无论这些损害如何发生以及基于何种责任理论（合同、严格责任或侵权行为
  （包括疏忽或其他）），即使已告知此类损害的可能性。

  注意：未授予任何第三方专利的许可。获得所需第三方专利许可证是被许可方的责任。

  关于上述术语的说明：
  1. 软件和HDL软件可互换使用。
  2. 二进制包括FPGA、仿真和物理形式，如硅芯片。
  3. 第2条允许在网页或其他电子表格上显示此类通知，而不是作为分发的一部分。
  原始BSD源码可从以下网址获取：
...
//                    设计信息
//  ----------------------------------------------------------------------------
//  文件            : i3c_slave_wrapper.v
//  组织            : MCO
//  标签             : 1.1.11
//  日期            : $Date: Tue Dec 10 18:39:13 2019 $
//  修订版本        : $Revision: 1.79 $
//
//  IP 名称         : i3c_slave_wrapper
//  描述            : MIPI I3C 从机包装器，纯粹在SCL时钟域中操作
//    这是从机包装器。它处理所有从机操作的部件，但不处理系统端。
//    外部系统包装器用于仅选择一个，具体取决于是否使用总线，
//    以及是否为主从或仅从机。这个从机包装器使用其实例化参数来确定实例化什么
//    并粘合在一起（例如HDR-DDR）。有一些必需的部分，如从机引擎、退出检测器、
//    从机DAA处理和最小CCC处理（用于DA使用）。
//    注意，CDC处理始终在此级别之上。
//
//  ----------------------------------------------------------------------------
//                    修订历史
...
//                    实现细节
//  ----------------------------------------------------------------------------
//  请参见程序员模型规范以及MIPI I3C规范
//
//    *****************
//    重要注意！！！
//    *****************
//    用适当的受保护逻辑替换CLOCK_SOURCE_xxx模块非常重要，
//    这些逻辑在时钟方面是无毛刺的，并且在FPGA的情况下是合适的时钟缓冲器。
//    后端(PD)工具应在约束中使用CLOCK_SOURCE_xx来获取时钟网络，
//    还有时钟关系，因此工具不会浪费精力。也就是说，
//    clk_SCL和clk_SCL_n当然通过相位相差180度（所以180相位）相关联。
//    clk_SDA相对于clk_SCL是异步的，但相对于pin_SCL_in也是"安全"的。
//  ----------------------------------------------------------------------------
